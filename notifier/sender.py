...
def format_message(top_coins: list[dict]) -> str:
    ...
    for i, c in enumerate(top_coins, 1):
        ...
        block = "\n".join(filter(None, [
            f"*{i}\\. {sym}* \\[{grade}\\]",
            "",
            f"👑 Score: `{c['final_score']:.0f}` | Flow: `{c.get('flow_score',0):.0f}` | Alpha: `{c.get('alpha_score',0):.0f}`",
            f"⚡ Pre\\-Exp: `{c.get('pre_exp_score',0):.0f}` | Liq: `{c.get('liquidity_score',50):.0f}` | Align: `{c.get('alignment_score',0):.0f}`",
            f"💰 Price: `{_fp(c['price'])}`",
            "",
            f"{p_em} Phase: *{_e(phase)}* | *{_e(c.get('pre_exp_dir','NEUTRAL'))}*",
            f"📐 TF: {_e(c.get('alignment_summary',''))}",
            "",
