...
def format_message(top_coins: list[dict], portfolio_usd: float = 1000.0) -> str:
...
def send_telegram(top_coins: list[dict], portfolio_usd: float = 1000.0) -> bool:
    if not TELEGRAM_TOKEN or not TELEGRAM_CHAT_ID:
        log.warning("Telegram not configured — printing to stdout")
        print(format_message(top_coins, portfolio_usd))
        return False

    text = format_message(top_coins, portfolio_usd)
