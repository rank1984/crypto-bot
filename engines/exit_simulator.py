"""
CRYPTO-BOT Elite — Exit Simulator v2
...
def simulate_trade_path(df, entry_price, sl, tp1, tp2, entry_ts, now_ts, timeout_hours=48):
    ...
    ambiguous_bar = 0
    exit_events = []
    is_closed = False  # 🆕 דגל שמציין אם העסקה הסתיימה באמת
    ...
    # יציאה בסוף הנתונים אם עדיין נשאר משהו פתוח
        ...
        if position > 0:
            ...
            # 🆕 קריטי: בודקים אם עברו 48 שעות אמיתיות
            if hours_since_entry >= timeout_hours:
                ...
                # רק נגמרו הנרות – העסקה עדיין רצה, לא סוגרים!
