...
def _cvd_reversal(df_5m: pd.DataFrame) -> tuple[bool, float]:
    """
    CVD התהפך — קונים הפכו למוכרים.
    מסתכל על 5 הנרות האחרונים לעומת ה-5 לפניהם.
    """
...
def _rs_breakdown(rs_1h: float, rs_4h: float) -> tuple[bool, float]:
    """
    RS מול BTC נשבר — המטבע נחלש ביחס לשוק.
    """
...
def _ema20_break(df_1h: pd.DataFrame) -> tuple[bool, float]:
    """
    הכי חשוב: Close מתחת EMA20 ב-1H = שבירת מבנה.
    בודק גם אם זה Close מתחת או רק wick.
    """
...
def _volume_exhaustion(df_5m: pd.DataFrame) -> tuple[bool, float]:
    """
    נפח מתייבש אחרי שיא — סימן שהמהלך מאבד כוח.
    Volume ב-5 נרות האחרונים < 50% מה-volume בשיא המהלך.
    """
...
def _whale_distribution(df_5m: pd.DataFrame) -> tuple[bool, float]:
    """
    לווייתנים מוכרים: נרות אדומים ענקיים = distribution.
    נרות אדומים גדולים ב-5 נרות האחרונים עם volume גבוה.
    """
...
def calc_exit_score(
    ...
    ----------
    df_5m, df_1h      : נרות
    rs_1h, rs_4h      : Relative Strength מול BTC
    oi_change_pct     : % שינוי OI
    current_gain_pct  : כמה % ברווח כרגע

    Returns
    -------
    {
        "exit_signal":   "HOLD" / "TRIM" / "EXIT",
        "exit_emoji":    str,
        "exit_desc":     str,
        "exit_score":    float,
        "confidence":    float,
