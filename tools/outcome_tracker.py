...
def _parse_ts(value):
    """
    Convert DB timestamp to naive UTC datetime.
    """
    if not value:
        return None
...
def update_outcomes():
    ...
    log.info(
        f"Outcome tracker V6: processing {len(rows)} open outcomes"
    )
    ...
    for row in rows:
        ...
        try:
            ...
            if t0 is None:
                log.warning(
                    f"{symbol}: invalid entry timestamp"
                )
                continue
            ...
            if df.empty:
                log.warning(
                    f"{symbol}: no candles after entry"
                )
                continue
            ...
            mfe_pct = (
                (max_high - entry) / entry * 100
            )

            mae_pct = (
                (min_low - entry) / entry * 100
            )
            ...
            """, (
                ...
                round(pnl_pct, 4),
                round(pnl_r, 4)
                if pnl_r is not None else None,
    ...
    log.info(
        f"Outcome tracker V6 complete: "
        f"{updated}/{len(rows)} updated"
    )
