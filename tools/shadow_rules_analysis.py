...
def run_analysis():
    ...
    print(f"Analyzing {len(trades)} BUY FINAL trades across {len(RULES)} rules\n")

    results = []
    for rule_name, rule_func in RULES.items():
        filtered = [t for t in trades if rule_func(t)]
        if not filtered:
            results.append((rule_name, 0, 0, 0, 0, 0))
            continue
        ...
        results.append((rule_name, cnt, round(tp1_rate*100,1), round(avg_pnl,2), round(avg_mfe,2), round(avg_mae,2)))
